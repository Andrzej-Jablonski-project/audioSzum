...
(function ($, window, document, undefined) {
	...
	Flashy.prototype = {
		...
		_showContent: function (item, direction) {
			...
			this.controls.$content.on('dragstart', 'img', function (e) {
				return false;
			});
			...
			if (w) {
				this.controls.$content.css('width', w);
			}
			if (h) {
				this.controls.$content.css('height', h);
			}
		},
		...
		_loadContent: function (type, item, href) {
			...
			switch (type) {
				case 'image':
					this._loadImage(def, item, href);
					break;
				case 'inline':
					this._loadInline(def, item, href);
					break;
				case 'ajax':
					this._loadAjax(def, item, href);
					break;
				case 'iframe':
					this._loadIframe(def, item, href);
					break;
				case 'video':
					this._loadVideo(def, item, href);
					break;
			}
